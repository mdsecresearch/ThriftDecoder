...
class ThriftMessage(object):
    def __init__(self, method, mtype, seqid, args, header=(), length=-1, proto='None'):
        self._method = method
        self._type = mtype
        self._seqid = seqid
    ...
    def read(cls, data,
        ...
        return cls(method, mtype, seqid, args, header, msglen, cls.proto_to_str(proto)), msglen
    ...
    @classmethod
    def proto_to_str(cls, proto):
        if isinstance(proto, TCompactProtocol):
            return 'compact'
        if isinstance(proto, TBinaryProtocol):
            return 'binary'
        if isinstance(proto, TJSONProtocol):
            return 'json'
