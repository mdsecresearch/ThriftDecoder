...
class MyList(list):
    @property
    def as_dict(self):
        return [field.as_dict if hasattr(field, 'as_dict') else field for field in self]
...
class MyDict(dict):
    @property
    def as_dict(self):
        return {x: y.as_dict if hasattr(y, 'as_dict') else y for x,y in self.iteritems()}
...
class ThriftStruct(object):
    ...
    def read_field_value(cls, proto, ftype,
        ...
        if ftype == TType.STRUCT:
            ...
            value = MyList()
            if read_values:
                values = MyList(_read(etype) for _ in xrange(size))
                value = MyDict()
                value['values'] = values
                value['etype'] = cls.field_type_to_str(etype)
            else:
            ...
            if read_values:
                ...
                for i in xrange(size):
                    ...
                    values[k] = v
                value['values'] = values
                value['ktype'] = cls.field_type_to_str(ktype)
                value['vtype'] = cls.field_type_to_str(vtype)
            else:
                for i in xrange(size):
                    proto.skip(ktype)
                    proto.skip(vtype)
